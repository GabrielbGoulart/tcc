...
org = "ufc"
bucket = "prices"
client = InfluxDBClient(url="http://18.191.223.172:8086", timeout=3600,token=token, org=org,enable_gzip=True)
write_api = client.write_api(write_options=SYNCHRONOUS)
    # write_options=WriteOptions(batch_size=1_000, flush_interval=1_000))
...
current_price =start_prices
def iter_bonds():
    insert_time =now
    for sec in range(3600):
        insert_time = insert_time - datetime.timedelta(0,1)
        print(insert_time)
        for i,stock_name in enumerate(stock_order):
            current_price[i] = current_price[i]+random_number()
            # print(stock_name,insert_time,current_price[i])
            yield Point("price").tag("bond", stock_name).field("value", current_price[i]).time(insert_time)

# iterable= iter_bonds()
# data = rx \
#     .from_iterable(iterable) 

# write_api.write(bucket="prices", record=data)
# write_api.close()
insert_time =now
for day in range(1,365):
    for sec in range(24):
        ...
        print(datetime.datetime.now(),insert_time)
